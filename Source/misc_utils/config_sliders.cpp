...
void ConfigSliders::Init()
{
	...
	mBoxSize.Set( sliders_config.background_width, 400 );
	
	// do we even need this?!
	mYPosition = sliders_config.background_spacing.y;

	// creating background
	{
		as::Sprite* background = new as::Sprite;
		as::Sprite* background_solid = as::LoadSprite( sliders_config.background_sprite );
		background_solid->SetName( "background_solid" );
		background_solid->SetColor( sliders_config.background_color );
		as::Sprite* background_dropshadow = as::LoadSprite( sliders_config.background_dropshadow );
...
void ConfigSliders::RecomputeBackgroundSize()
{
	...
	ResizeBackground( (float)element_count * sliders_config.element_spacing.y + 2.f * sliders_config.background_spacing.y );
}
//-----------------------------------------------------------------------------

as::Sprite* ConfigSliders::CreateLine()
{
	if( sliders_config.use_background_lines == false ) return NULL;
	if( mCanCreateLines == false ) return NULL;

	const types::vector2 position( 
		0,
		GetElementCount() * sliders_config.element_spacing.y + sliders_config.background_spacing.y );

	as::Sprite* line = as::LoadSprite( sliders_config.background_line_sprite );
	line->MoveTo( position + sliders_config.background_line_position );
	line->Resize( sliders_config.background_line_size.x, sliders_config.background_line_size.y );
	line->SetColor( sliders_config.background_line_color );
...
void ConfigSliders::AddComponentHeader( const std::string& component_name, bool remove_me_button, void* extra_data )
{
	...
	const types::vector2 position( 
		sliders_config.background_spacing.x, 
		GetElementCount() * sliders_config.element_spacing.y + sliders_config.background_spacing.y );
	...
	if( remove_me_button )
	{
		...
		button_sprite->SetColor( sliders_config.checkbox_back_color );
...
as::Sprite*	ConfigSliders::CreateLabel( const std::string& name )
{
	...
	as::TextSprite* t = as::LoadTextSprite( sliders_config.text_font_file );
	t->MoveTo( sliders_config.text_extra_position );
	t->SetColor( sliders_config.text_color );
	t->SetText( name );
	t->SetName( "text_name" );
	text_element->addChild( t );

	// text_element fade off part
	if( sliders_config.text_use_fadeoff ) 
	{
		as::Sprite* fadeoff_part = as::LoadSprite( sliders_config.text_fadeoff_sprite );
		fadeoff_part->SetColor( sliders_config.background_color );
		fadeoff_part->MoveTo( sliders_config.text_fadeoff_position );
		text_element->addChild( fadeoff_part );

		as::Sprite* fadeoff_extra = as::LoadSprite( sliders_config.text_fadeoff_extra_sprite );
		fadeoff_extra->SetColor( sliders_config.background_color  );
		fadeoff_extra->MoveTo( sliders_config.text_fadeoff_position + types::vector2( fadeoff_part->GetTextureSize().x, 0 ) );
		fadeoff_extra->Resize( sliders_config.text_fadeoff_extra_size.x, sliders_config.text_fadeoff_extra_size.y );
...
as::Sprite*	ConfigSliders::CreateHeader( const std::string& name )
{
	...
	as::TextSprite* t = as::LoadTextSprite( sliders_config.header_font_file );
	t->MoveTo( sliders_config.header_extra_position );
	t->SetColor( sliders_config.header_color );
	t->SetText( name );
	t->SetName( "text_name" );
	text_element->addChild( t );
...
as::Sprite* ConfigSliders::CreateTextValue( const std::string& text )
{
	...
	text_value->MoveTo( position + sliders_config.text_value_position );
...
SimpleTextbox*	ConfigSliders::CreateTextboxValue( const std::string& name, const std::string& value )
{
	...
	// text_sprite->MoveTo( position + sliders_config.text_value_position );
	...
	simple_textbox->MoveTo( position + sliders_config.text_value_position );
...
SimpleSlider* ConfigSliders::CreateSlider( const std::string& name ) 
{
	...
	{
		as::Sprite* slider_back = as::LoadSprite( sliders_config.slider_back_sprite );
		slider_back->MoveTo( position + sliders_config.slider_back_position );
		slider_back->SetColor( sliders_config.slider_color );
	...
	as::Sprite* slider_front = as::LoadSprite( sliders_config.slider_front_sprite );
	as::Sprite* slider_front_stroke = as::LoadSprite( sliders_config.slider_front_stroke_sprite );

	slider_front->SetCenterOffset( types::vector2( 0.5f * slider_front->GetTextureSize().x, 0 ) );
	slider_front->SetColor( sliders_config.slider_color );
	slider_front_stroke->SetCenterOffset( types::vector2( 0.5f * slider_front_stroke->GetTextureSize().x, 0 ) );
	slider_front_stroke->SetColor( sliders_config.slider_stroke_color );
	...
	slider->SetName( name );
	slider->mStartPos = position + sliders_config.slider_front_start_pos;
	slider->mEndPos = position + sliders_config.slider_front_end_pos;
...
SimpleCheckbox* ConfigSliders::CreateCheckbox( const std::string& name, bool value )
{
	...
	as::Sprite* checkbox_sprite = as::LoadSprite( sliders_config.checkbox_back_sprite );
	as::Sprite* checkbox_overlay = as::LoadSprite( sliders_config.checkbox_front_sprite );
	checkbox_sprite->SetColor( sliders_config.checkbox_back_color );
	checkbox_overlay->SetColor( sliders_config.checkbox_front_color );
	...
	simple_checkbox->MoveTo( position + sliders_config.checkbox_position );
...
SimpleTextbox*	ConfigSliders::CreateTextbox( const std::string& name, const std::string& value )
{
	...
	// ---------------------
	as::Sprite* back_textbox = as::LoadSprite( sliders_config.textbox_back_sprite );
	back_textbox->MoveTo( sliders_config.textbox_back_sprite_position );
	

	as::TextSprite* text_sprite = as::LoadTextSprite( sliders_config.textbox_font );
	text_sprite->MoveTo( sliders_config.textbox_font_position );
	text_sprite->SetName( "text_element" );
	text_sprite->SetColor( sliders_config.textbox_font_color );
	...
	simple_textbox->MoveTo( position + sliders_config.checkbox_position );
	this->addChild( simple_textbox );
	mSliders.push_back( simple_textbox );
...
SimpleDropDown* ConfigSliders::CreateDropDown( const std::string& name, const std::string& box_name, const std::vector< std::string >& values )
{
	...
	// ----
	as::Sprite* dropdown_top = as::LoadSprite( sliders_config.dropdown_top_sprite );
	dropdown_top->MoveTo( sliders_config.dropdown_top_sprite_position );
	

	as::TextSprite* text_sprite = as::LoadTextSprite( sliders_config.dropdown_font );
	text_sprite->MoveTo( sliders_config.dropdown_font_position );
	text_sprite->SetName( "text_element" );
	text_sprite->SetColor( sliders_config.dropdown_font_color );
	...
	{
		as::Sprite* dropdown_item = as::LoadSprite( sliders_config.dropdown_item_background_sprite );
		dropdown_item->MoveTo( sliders_config.dropdown_item_start_pos + ( sliders_config.dropdown_item_position_size * (float)i ) );
		dropdown_item->SetName( values[ i ] );

		as::TextSprite* text_item = as::LoadTextSprite( sliders_config.dropdown_font );
		text_item->MoveTo( sliders_config.dropdown_item_font_position );
		text_item->SetName( "text_element" );
		text_item->SetColor( sliders_config.dropdown_item_font_color );
	...
	simple_dropdown->MoveTo( position + sliders_config.dropdown_position );
